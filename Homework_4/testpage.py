...
class OperationsHelper(BasePage):
    ...
    def enter_contact_email(self, word):
        self.enter_text_into_field(TestSearchLocators.ids['LOCATOR_CONTACT_EMAIL_FIELD'], word,
    ...
    def enter_post_title(self, word):
        self.enter_text_into_field(TestSearchLocators.ids["LOCATOR_TITLE_FIELD"], word, description="test post title")
    ...
    def click_save_post_button(self):
        self.click_button(TestSearchLocators.ids["LOCATOR_SAVE_POST_BTN"], description="save new test post")
    ...
    def get_post_description_list(self):
        post = requests.post(url=testdata["path"],
                             data={"username": testdata["username"], "password": testdata["pass"]})
        token = (post.json()["token"])
        get_post = requests.get(url=testdata["post_path"], headers={"X-Auth-Token": token})
        description_list = [item["description"] for item in get_post.json()["data"]]
